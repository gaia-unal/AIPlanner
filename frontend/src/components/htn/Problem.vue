<template>
  <div>
    ...
    <div class="form-group row px-4">
      ...
      <button class="btn btn-success col-md-1.5" style="margin-top: 32px" @click="addTask">Agregar</button>
    </div>
    <ul class="list-group my-3 px-4" v-if="problem.tasks.length > 0">
      <li class="list-group-item" v-for="(item, index) in problem.tasks" :key="item.id">
      <div class="form-group row" style="margin-bottom: 0px">
        <div class="col-md-4 my-auto">
          {{ item.name }}
        </div>
        ...
        <button class="btn btn-outline-danger col-md-1.5" style="margin-left: 8px" @click="deleteTask(index)">Eliminar</button>
      </div>
      </li>
    </ul>
  </div>
  ...
  methods: {
    ...
    addTask () {
      if (this.finalAf.length > 0 && this.finalVar.length > 0 && this.problem.name.length > 0) {
        ...
        this.problem.tasks.push({
          name: this.finalAf,
          value: this.finalVar.split(',')
        })
        this.finalAf = ''
        this.finalVar = ''
      } else {
    ...
    deleteTask (index) {
      this.problem.tasks.splice(index, 1)
    }
  }
}
</script>
